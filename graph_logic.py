...
ROOM_RULES_CONFIG = {
    "living_room": {
        "core": "sofa",
        "required": ["sofa", "coffee_table", "tv_stand", "tv"],
        "optional": ["rug", "armchair", "lamp", "bookshelf"]
    },
    "bedroom": {
        "core": "bed",
...
def get_style_score(m1, m2):
    ...
    if not g1 or not g2: return 0.2
    if g1 == g2: return 1.0 if s1 == s2 else 0.9
...
def calculate_edge_weight(node1, node2):
    ...
    total = (style_sim * WEIGHT_FACTORS["style"] + color_sim * WEIGHT_FACTORS["color"] +
...
def build_scene_graph(candidates_dict):
    ...
    for cat, items in candidates_dict.items():
        for item in items:
            ...
            G.add_node(
                f"{cat}_{item['uid']}", category=cat, uid=item['uid'],
...
def solve_optimal_subgraph(G, room_type):
    ...
    if not core_nodes: return []

    best_core = select_best_core(G, core_nodes)
    selected_ids = [best_core]
    selected_nodes_data = [G.nodes[best_core]]

    color_history = Counter([c for c in G.nodes[best_core]['meta'].get('color', '').split(',') if c.strip()])
    ...
    for cat in [c for c in all_cats if c not in {n['category'] for n in selected_nodes_data}]:
        ...
        for cand in candidates:
            ...
            for c in [c for c in cand_data['meta'].get('color', '').split(',') if c.strip()]:
                if color_history[c] >= 2: rel_score -= 15
        ...
        if best_cand and (max_score > 0 or cat in rules["required"]):
            ...
            color_history.update([c for c in G.nodes[best_cand]['meta'].get('color', '').split(',') if c.strip()])
